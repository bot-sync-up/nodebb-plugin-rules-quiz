...
function poolForMode(allQuestions, settings, mode) {
  ...
  if (pool.mode !== 'single_tagged' && pool.mode !== 'separate') return allQuestions || [];
  let tag = '';
  if (mode === 'post') tag = pool.postTag || '';
  else if (mode === 'topic') tag = pool.topicTag || '';
  else tag = pool.onboardingTag || '';
  if (!tag) return allQuestions || [];
  const filtered = filterQuestionsByTag(allQuestions || [], tag);
  if (filtered.length === 0 && pool.mode === 'single_tagged') {
    // Fresh install / untagged seed — keep gate working by falling back.
    return allQuestions || [];
  }
  return filtered;
}
