...
function setup(params) {
  ...
  // CSRF middleware: NodeBB's setupApiRoute binds CSRF internally, but the
  // raw-router fallback (NodeBB ≤ 2.5 or a future signature change) would
  // skip it. Pull whichever CSRF middleware exists and prepend it so the
  // fallback path is also protected against cross-site write requests.
  const csrf = [
    middleware.applyCSRF,
    middleware.csrf,
    middleware.applyCSRFasync,
  ].find((fn) => typeof fn === 'function');
  const csrfChain = csrf ? [csrf] : [];
  const authChain = authenticate ? [authenticate] : [];
  const userChain = [].concat(
    csrfChain,
  ...
  const adminCheck = (middleware.admin && typeof middleware.admin.checkPrivileges === 'function')
    ...
    : null;
  const adminChain = [].concat(csrfChain, authChain, adminCheck ? [adminCheck] : []);
