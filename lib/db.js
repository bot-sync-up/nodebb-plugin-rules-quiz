...
async function getSettings() {
  ...
  // Migrations for values removed/renamed across versions:
  // - 'modal_soft' block mode was removed in v0.8.0 (it enforced nothing).
  //   Treat any stored value as 'block_write' so gates still enforce.
  if (merged.blockMode === 'modal_soft' || !merged.blockMode) {
    merged.blockMode = 'block_write';
  }
  // - pool.mode 'separate' is unimplemented; fall back to 'single_tagged'
  //   so an admin who picked it doesn't get empty quizzes.
  if (merged.pool && merged.pool.mode === 'separate') {
    merged.pool.mode = 'single_tagged';
  }
  return merged;
}
...
async function setUserState(uid, partial) {
  if (uid === undefined || uid === null) return defaultUserState();
  const p = partial || {};
  // IMPORTANT: write ONLY the fields present in `partial`, not the whole
  // reconstructed hash. The previous version read-merged-then-setObject'd
  // every field, which clobbered concurrent atomic increments
  // (`incrUserField`) and racing partial writes from other code paths.
  // Per-field writes mean two callers updating DIFFERENT fields never
  // step on each other, and the counter increments stay atomic.
  const toStore = {};
  if (Object.prototype.hasOwnProperty.call(p, 'status')) toStore.status = p.status;
  if (Object.prototype.hasOwnProperty.call(p, 'attempts')) toStore.attempts = p.attempts;
  if (Object.prototype.hasOwnProperty.call(p, 'lastAttemptAt')) toStore.lastAttemptAt = p.lastAttemptAt;
  if (Object.prototype.hasOwnProperty.call(p, 'passedAt')) toStore.passedAt = p.passedAt;
  if (Object.prototype.hasOwnProperty.call(p, 'gateAck')) toStore.gateAck = p.gateAck ? 1 : 0;
  if (Object.prototype.hasOwnProperty.call(p, 'postsCreated')) toStore.postsCreated = p.postsCreated || 0;
  if (Object.prototype.hasOwnProperty.call(p, 'topicsCreated')) toStore.topicsCreated = p.topicsCreated || 0;
  if (Object.prototype.hasOwnProperty.call(p, 'postTokenExp')) toStore.postTokenExp = p.postTokenExp || 0;
  if (Object.prototype.hasOwnProperty.call(p, 'topicTokenExp')) toStore.topicTokenExp = p.topicTokenExp || 0;
  if (Object.prototype.hasOwnProperty.call(p, 'returnTo')) toStore.returnTo = p.returnTo || '';
  if (Object.prototype.hasOwnProperty.call(p, 'lastGateAt')) toStore.lastGateAt = p.lastGateAt || 0;
  if (Object.prototype.hasOwnProperty.call(p, 'lastGateKind')) toStore.lastGateKind = p.lastGateKind || '';
  if (Object.keys(toStore).length > 0) {
    await db.setObject(KEYS.user(uid), toStore);
  }
  // Return the full merged view for callers that want it.
  const current = await getUserState(uid);
  return current;
}

/**
 * Atomically increment a numeric field on the user-state hash. Use this for
 * counters (postsCreated / topicsCreated) so concurrent gate passes never
 ...
 */
async function incrUserField(uid, field, by) {
  if (uid === undefined || uid === null) return 0;
  const amount = Number.isFinite(by) ? by : 1;
  // db.incrObjectFieldBy where available (atomic +N); else fall back to the
  // single-step incrObjectField in a tiny loop. Both are atomic per call.
  if (typeof db.incrObjectFieldBy === 'function') {
    return db.incrObjectFieldBy(KEYS.user(uid), field, amount);
  }
  let result = 0;
  for (let i = 0; i < Math.abs(amount); i++) {
    // eslint-disable-next-line no-await-in-loop
    result = await db.incrObjectField(KEYS.user(uid), field);
  }
  return result;
}
...
async function finishAttempt(aid, payload) {
  ...
  // daily aggregate — atomic per-field increment (split fields
  // "<date>:passed" / "<date>:failed"). The legacy single-JSON-field
  // format is still read by getStats for backward compatibility.
  const dateKey = ymd(finishedAt);
  const dailyField = dateKey + (passed ? ':passed' : ':failed');
  try {
    if (typeof db.incrObjectField === 'function') {
      await db.incrObjectField(KEYS.statsDaily, dailyField);
    } else {
      const cur = await db.getObjectField(KEYS.statsDaily, dailyField);
      await db.setObjectField(KEYS.statsDaily, dailyField, toInt(cur, 0) + 1);
    }
  } catch (_) { /* non-fatal */ }
...
async function getStats(range) {
  ...
  if (allDays && typeof allDays === 'object') {
    // Collapse both storage formats into a per-date {passed, failed} map:
    //   - legacy: field "<date>"            -> JSON string {passed, failed}
    //   - new:    fields "<date>:passed" / "<date>:failed" -> integers
    const byDate = {};
    Object.keys(allDays).forEach((rawField) => {
      let date = rawField;
      let which = null;
      if (rawField.endsWith(':passed')) { date = rawField.slice(0, -7); which = 'passed'; }
      else if (rawField.endsWith(':failed')) { date = rawField.slice(0, -7); which = 'failed'; }
      if (!byDate[date]) byDate[date] = { passed: 0, failed: 0 };
      if (which) {
        byDate[date][which] += toInt(allDays[rawField], 0);
      } else {
        const entry = safeParse(allDays[rawField], { passed: 0, failed: 0 });
        byDate[date].passed += toInt(entry.passed, 0);
        byDate[date].failed += toInt(entry.failed, 0);
      }
    });
    ...
    for (let i = 0; i < dates.length; i += 1) {
      ...
      const failed = byDate[date].failed;
      totalPassed += passed;
      totalFailed += failed;
