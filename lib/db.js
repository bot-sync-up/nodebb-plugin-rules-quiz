...
async function getUserState(uid) {
  ...
  const hash = await db.getObjectFields(
    KEYS.user(uid),
    ['status', 'attempts', 'lastAttemptAt', 'passedAt', 'gateAck']
  );
...
async function setUserState(uid, partial) {
  ...
  // store gateAck as a boolean string for consistency across backends
  const toStore = {
 ...
 * @param {{ answers: Array<{qid:number,correct:boolean,given:*}>, score:number, total:number, passed:boolean }} payload
